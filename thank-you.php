<?php
$page ='thank-you';
?>

<?php include 'inc/head.php'; ?>

<title>Ivas</title>

<meta name="robots" content="noindex, nofollow">

<meta name="description" content="" />
<link rel="canonical" href="" />
<!-- Event snippet for Submit lead form conversion page -->
<script>
  gtag('event', 'conversion', {'send_to': 'AW-11100511761/GL5zCLvhq5QYEJG8ka0p'});
</script>


<?php include 'inc/header.php'; ?>

 
		  
<section class="contact-us inquiry-model my-5">
    <div class="container">
        <center>
		<div id="content">
			<div class="container thank-you">
			
			<h2>Thank You !</h2>
			<p class="lead">We will get back to you soon.</p>

			</div>
			</div>
		</center>
        <div class="row text-center form-policy">
            <div class="col-sm-12">
           <p>Thank you for taking the time to fill out the form. We value your interest in our products.<br/>Our team will get in touch with you shortly to understand your requirements.</p>
  		
		<p class="text-center"><a class="btn btn-lg btn-dark rounded-circle" href="index.php">Back to home</a></p>
            </div>
        </div>
    </div>
</section>

<?php include 'inc/footer-js.php'; ?>
<?php include 'inc/footer.php'; ?>

<script> 
setTimeout(function()
{ 
window.location = "index.php"; 
},10000);
</script>